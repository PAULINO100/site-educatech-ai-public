...
export default function FreeKeyGenerator() {
  ...
  return (
    <div className="max-w-md mx-auto glass-bright p-8 rounded-3xl border border-indigo-500/20 shadow-2xl glow-indigo">
      ...
      <p className="text-slate-400 text-xs mb-6 text-center leading-relaxed">
        Obtenha uma chave anônima e instantânea para ativar a governança Tier A+ em sua IDE.
      </p>
      ...
      {!key ? (
        <button
          onClick={generateKey}
          disabled={loading}
          className="w-full py-4 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-bold transition-all shadow-lg shadow-indigo-500/20 active:scale-95 disabled:opacity-50"
        >
          {loading ? 'Gerando...' : 'Gerar Chave de Acesso'}
        </button>
      ) : (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
          <div className="bg-slate-950/80 rounded-xl p-4 border border-slate-800 font-mono text-sm break-all text-indigo-400 text-center">
            {key}
          </div>
          <button
      ...
      <div className="mt-8 pt-6 border-t border-slate-800/50">
        ...
        <ol className="text-[11px] text-slate-400 space-y-2 text-left list-decimal list-inside">
          ...
          <li>Abra sua IDE (VS Code / Cursor).</li>
          <li>Inicie o motor Certus para ativar a segurança e o monitoramento soberano.</li>
        </ol>
      </div>
    </div>
  )
